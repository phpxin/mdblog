...
var (
	defaultImgs = []string{
		"//upload-dream128.oss-cn-beijing.aliyuncs.com/default1.png",
		"//upload-dream128.oss-cn-beijing.aliyuncs.com/default2.png",
		"//upload-dream128.oss-cn-beijing.aliyuncs.com/default.jpg",
	}
)
...
func GetDocsByPage(page int32, limit int32) ([]*Doc, int32) {
	...
	start := (page - 1) * limit
	db.Where("status=?", DOC_STATUS_NORMAL).Order("edited_at desc").Offset(start).Limit(limit).Find(&results)
